...
class ReaderAdapterTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        $ra1 = new ReaderAdapter(self::$data, function ($item) { return $item; });
        $this->assertTrue(!!$ra1);
        
        $f = tempnam('/tmp', 'tmp');
        $ra2 = new ReaderAdapter(new Csv($f), function ($item) { return $item; });
        $this->assertTrue(!!$ra2);
        @unlink($f);
    }
    ...
    {
        $ra = new ReaderAdapter(new \DateTime(), function ($item) { return $item; });
    }
    ...
    {
        $ra = new ReaderAdapter(new Csv('adsfasdf.csv'), function ($item) { return $item; });
    }
    
    public function testEquality()
    {
        $ra = new ReaderAdapter(self::$data, function ($item) { return $item; });
        $ra2 = [];
        foreach ( $ra as $item ) {
            $ra2[] = $item;
        }
    ...
    {
        $ra = new ReaderAdapter(self::$data, function ($item) { return $item * 2; });
        $ra2 = [];
        foreach ( $ra as $item ) {
            $ra2[] = $item;
        }
